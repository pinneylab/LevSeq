...
def demux_fastq(
    ...
    prompt = ' '.join([
        str(executable_path),
        '-f {}'.format(str(file_to_fastq)),
        '-d {}'.format(str(result_folder)),
        '-b {}'.format(str(barcode_path)),
...
def call_variant(experiment_name, experiment_folder, template_fasta, filtered_barcodes, calling_threshold):
    try:
        vc = VariantCaller(
            experiment_name,
            experiment_folder,
        ...
        variant_df = vc.get_variant_df(threshold=calling_threshold, min_depth=5)
        ...
        if not cl_args["skip_variantcalling"]:
            try:
                variant_result = call_variant(
                    f"{name}", name_folder, temp_fasta_path, barcode_path, cl_args['calling_threshold']
                )
...
def run_LevSeq(cl_args, tqdm_fn=tqdm.tqdm):
    ...
    try:
        layout = generate_platemaps(
            ...
            offset=cl_args['position_offset']
        )
