FROM rockylinux:8.10 AS build-demultiplex
...
# # Create levseq environment and install dependencies
# RUN conda create --name levseq python=3.12 -y
# RUN conda activate levseq
# RUN conda install -c bioconda -c conda-forge samtools
# RUN conda install -c bioconda -c conda-forge minimap2
# RUN conda install -c conda-forge gcc=13 gxx=13 zlib


# # Compile the demultiplexing code and copy
# RUN cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_C_COMPILER=gcc -DCMAKE_CXX_COMPILER=g++ ../source/
# COPY 
